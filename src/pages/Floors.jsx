...
const Floors = () => {
    ...
    const getList = async () => {
        try {
            setLoading(true);
            const { data } = await API.get("/admin/floors", { params: { page, per_page: perPage } });
            setList(data?.data?.floors || data?.data || []);
    ...
    const handleConfirmDelete = async () => {
        try {
            setDelLoading(true);
            const resp = await API.delete(`/admin/floors/${reqId}`);
    ...
    const submitForm = async (e) => {
        ...
        try {
            ...
            if (formMode === "edit" && selectedItem) {
                resp = await API.put(`/admin/floors/${selectedItem.id}`, payload);
            } else {
                resp = await API.post("/admin/floors", payload);
            }
    ...
    return (
        <div className="main">
            <Sidebar />
            <Header pageTitle={t("floors.plural")} />
                        ...
                        <div className="table-header">
                            <div className="table-header__top">
                                <div className="table-header__title-wrap">
                                    <h3 className="table-title">{t("floors.plural")}</h3>
                                </div>

                                <div>
                                    <button className="btn btn-primary" onClick={openAddPopup}>
                                        {t("common.add")} {t("floors.singular")}
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="popup-overlay">
                            <div className="popup-container modal-md">
                                <div className="popup-header">
                                    <h2>{formMode === "edit" ? `${t("common.edit")} ${t("floors.singular")}` : `${t("common.add")} ${t("floors.singular")}`}</h2>

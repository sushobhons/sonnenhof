...
const Documents = () => {
  ...
  const getList = async () => {
    try {
      ...
      const items = payload.documents || payload || [];

      // normalize relations and build file_url if available
      const normalized = items.map((it) => {
        ...
        const fileUrl = it.file_url || it.file_path || null;
  ...
  const submitForm = async (e) => {
    ...
    if (!formData.title) {
      toast.error(t("common.validation.required") || "Title is required", { theme: "dark", autoClose: 2000 });
      return;
    }
    ...
    try {
      ...
      if (formMode === "edit" && selectedItem) {
        response = await API.post(`/admin/documents/${selectedItem.id}`, fd, { headers: { "Content-Type": "multipart/form-data", "X-HTTP-Method-Override": "PUT" } });
      } else {
        response = await API.post("/admin/documents", fd, { headers: { "Content-Type": "multipart/form-data" } });
      }
  ...
  const handleConfirmDelete = async () => {
    try {
      setDelLoading(true);
      const resp = await API.delete(`/admin/documents/${reqId}`);
  ...
  return (
    <div className="main">
      <Sidebar />
      <Header pageTitle={t("documents.plural")} />
          ...
          <div className="table-container">
            <div className="table-header">
              ...
              <div className="table-header__top">
                <div className="table-header__title-wrap">
                  <h3 className="table-title" style={{ margin: 0 }}>{t("documents.plural")}</h3>
                </div>
                <div>
            ...
            <div className="table-wrapper">
              <table className="table">
                <thead>
                  <tr>
                    ...
                    <th style={{ cursor: "pointer" }} onClick={() => handleSort("title")}>
                      {t("documents.title") || "Title"} {sortIndicator("title")}
                    </th>
                ...
                <tbody>
                  {loading ? (
                    <tr><td colSpan={10}>{t("common.loading")}...</td></tr>
                  ) : list.length ? (
                    list.map(item => (
                      <tr key={item.id}>
                        ...
                        <td>
                          {/* {item.file_url || item.file_path ? (
                            <a href={item.file_url || item.file_path} target="_blank" rel="noreferrer" className="btn-action btn-view"><img src={newTabImg} alt="View File" style={{ width: 24 }} /></a>
                          ) : "-"} */}
                    ...
                    <tr><td colSpan={10} style={{ textAlign: "center" }}>{t("common.result.no_record")}</td></tr>
                  )}
                </tbody>
              </table>
            </div>
            ...
            <div className="popup-overlay">
              <div className="popup-container modal-md">
                <div className="popup-header">
                  <h2>{formMode === "edit" ? `${t("common.edit")} ${t("documents.singular")}` : `${t("common.add")} ${t("documents.singular")}`}</h2>
                ...
                <form onSubmit={submitForm}>
                  ...
                  <div className="popup-footer">
                    <button type="submit" className="btn btn-success" disabled={isSubmitting}>{isSubmitting ? <CircularProgress size={20} style={{ color: "#0f4166" }} /> : t("common.save")}</button>
                    <button type="button" className="btn btn-secondary" onClick={closePopup} style={{ marginLeft: 8 }}>{t("common.cancel")}</button>
                  </div>
                </form>
              </div>
            </div>
              ...
              <div className="popup-container modal-sm warning-modal-box">
                ...
                <div className="popup-body">
                  <div className="form-container" style={{ textAlign: "center" }}>
                    ...
                    <p>{t("documents.delete_confirm") || t("common.delete_confirm")}</p>
                  </div>
                </div>
